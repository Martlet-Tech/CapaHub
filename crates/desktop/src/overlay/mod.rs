...
// Raw Win32 handles: only accessed from main thread, safe to share.
...
pub fn init() {
    *MANAGER.lock().unwrap() = Some(OverlayManager { next_id: 1, windows: HashMap::new() });
}
...
pub fn handle_cmd(json: &str) -> Result<u32, String> {
    ...
    match cmd {
        "create" => {
            ...
            mgr.windows.insert(id, OverlayWindow { hwnd, w, h, hdc, bitmap });
            Ok(id)
        }
        "draw_line" => {
            let h = v["h"].as_u64().unwrap_or(0) as u32;
            let win = mgr.windows.get(&h).ok_or("invalid handle")?;
            draw_line_on(win, v["x1"].as_i64().unwrap_or(0) as i32, v["y1"].as_i64().unwrap_or(0) as i32,
                         v["x2"].as_i64().unwrap_or(0) as i32, v["y2"].as_i64().unwrap_or(0) as i32,
...
fn create_overlay(x: i32, y: i32, w: i32, h: i32) -> Result<(Hwnd, Hdc, Hbmp), String> {
    ...
    unsafe {
        RegisterClassW(&WNDCLASSW {
            style: 0, lpfnWndProc: Some(DefWindowProcW),
            cbClsExtra: 0, cbWndExtra: 0, hInstance: hinst,
            hIcon: std::ptr::null_mut(), hCursor: std::ptr::null_mut(),
            hbrBackground: std::ptr::null_mut(), lpszMenuName: std::ptr::null_mut(),
            lpszClassName: class.as_ptr() as *mut u16,
        });
    }
    let hwnd = unsafe {
        CreateWindowExW(WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOPMOST | WS_EX_TOOLWINDOW,
            class.as_ptr(), std::ptr::null(), WS_POPUP, x, y, w, h,
    ...
    let (hdc, bitmap) = unsafe {
        ...
        FillRect(hdc, &RECT { left:0, top:0, right:w, bottom:h }, brush);
        DeleteObject(brush);
        (hdc, bmp)
    };
...
fn draw_line_on(win: &OverlayWindow, x1: i32, y1: i32, x2: i32, y2: i32, color: u32, thickness: i32) -> Result<(), String> {
    ...
    unsafe {
        ...
        let old = SelectObject(win.hdc.0, pen);
        MoveToEx(win.hdc.0, x1, y1, std::ptr::null_mut());
        LineTo(win.hdc.0, x2, y2);
        SelectObject(win.hdc.0, old);
        DeleteObject(pen);
    }
    flush(win)
}
...
fn clear_window(win: &OverlayWindow) -> Result<(), String> {
    unsafe {
        let brush = CreateSolidBrush(0x00000000);
        FillRect(win.hdc.0, &RECT { left:0,top:0,right:win.w,bottom:win.h }, brush);
        DeleteObject(brush);
    }
    flush(win)
}
...
fn destroy_overlay(win: OverlayWindow) {
    unsafe {
        DeleteDC(win.hdc.0);
        DeleteObject(win.bitmap.0);
        DestroyWindow(win.hwnd.0);
    }
}
